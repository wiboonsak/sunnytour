...
<script type="text/javascript">
    ...
    "search" : false
     ...
     function Add() {
        ...
        if (name == '') {
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url('PackageCMS/addslideticket') ?>',
                processData: false,
                contentType: false,
                data: postData,
                success: function (data, status) {
                    ...
                    if (status == 'success') {
                        ...
                        setTimeout(function () {
                            window.location.href = "<?php echo base_url('PackageCMS/ticketslide') ?>" ;
                        }, 2000);
                    } else {
                        swal({
    ...
    //----------------------------------
   function  loadSeason() {
        $.post('<?php echo base_url('PackageCMS/loadslideticket') ?>', {}, function (data) {
    ...
    function updateOrder(dataID,FieldName,changeValue){
		...
		console.log('returnValue:-'+returnValue)
		 if((changeValue=='')){
			...
			$.post('<?php echo base_url('PackageCMS/updateOrderticket')?>',{ dataID:dataID , FieldName:FieldName , changeValue:changeValue }
				  ,function(data){ 
						if(data==1){
										swal({
                    ...
                    setTimeout(function(){ window.location.href = "<?php echo base_url('PackageCMS/ticketslide')?>"; }, 2000);
     ...
     //----------------------
	function delete_data(dataID,table,img){  
		swal({
		   ...
		   $.post('<?php echo base_url('PackageCMS/deleteDataticket')?>' , { dataID : dataID , table : table, img : img }, 
			function(data){
				if(data==1){ 
                	swal({
